...
备用脚本1：（实测建议先用2）
wget -q -N --no-check-certificate https://raw.githubusercontent.com/HyoukaChitandaEru/AR-B-P-B-SSR/master/install.sh && bash install.sh develop

备用脚本2（支持离线安装，此方法可用于网络情况不好，导致缺少文件的情况.亦可下载该脚本，用于备份）：（支持多种系统Ubuntu 18.04 x64等）
wget -q -N --no-check-certificate https://raw.githubusercontent.com/HyoukaChitandaEru/SSR/master/install.sh && bash install.sh
———————————————————代码分割线————————————————
复制上面的代码到VPS服务器里，复制代码用鼠标右键的复制，然后在vps里面右键粘贴进去，因为ctrl+c和ctrl+v无效。接着按回车键，脚本会自动安装，以后只需要运行这个快捷命令就可以出现下图的界面进行设置，快捷管理命令为：bash ssr.sh（第一个脚本）   ssr（备用脚本）
...
第三步：一键加速VPS服务器（谷歌BBR算法已经开源，各发行版大多已经安装了BBR，这步可省略。）
...
yum -y install wget
wget --no-check-certificate https://raw.githubusercontent.com/HyoukaChitandaEru/BBR/master/Googlebbr.sh
chmod +x bbr.sh
./bbr.sh（不要复制回车符）
